...
function loadCategories(){
    fetch(BASE_URL + "categories")
      ...
      .then((data) =>  renderCategories(data)).catch((error) => console.log('Error:', error ));
}
...
function renderProducts(products){
  ...
  for (let i = 0; i < products.length; i++) {
    productListHtml +=
      ...
      '<img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTwCmk3ev3tl5UvK3EmZBZy7CWXU-Ip3iYbOg&usqp=CAU" >' +
      "</div>" +
      '<div class="product-name text-center">' + products[i].name + '</div>' +
      '<div class="product-price text-center">&#8377;' + products[i].cost + '</div>' + '</a>'
  }
